...
SYSTEM_LIST="~/routing_tables/systems.txt"
...
mkdir ~/routing_tables/routing_tables_${TIMESTAMP}

# Loop through the list of systems
for system in "${systems[@]}"
do
    # Export the routing table from the remote system
    ssh -i ${SSH_KEY} ${SSH_USER}@$system "netstat -rn" > ~/routing_tables/routing_tables_${TIMESTAMP}/$system.txt

    # Print a message to confirm the export
    echo "Routing table exported from $system"
done

scp -i ${BCK_KEY} -r ~/routing_tables/routing_tables_${TIMESTAMP} ${BCK_USER}@${BCK_SERVER}:${BCK_PATH}
